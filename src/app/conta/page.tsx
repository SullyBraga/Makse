...
import { Package, User, LogOut, Edit3, Check, X, Eye, EyeOff, Scissors, RefreshCw } from 'lucide-react'
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '2rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', overflow: 'hidden' }}>
              {[
                { icon: <Package size={14} />, label: 'Meus Pedidos', href: '/conta' },
                { icon: <User size={14} />, label: 'Dados Pessoais', href: '#dados', onClick: () => setEditing(true) },
              ].map((item, i) => (
                <a
                  key={item.label}
                  href={item.href}
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1.25rem', width: '100%', background: 'none', border: 'none', fontSize: '0.82rem', color: 'var(--text-muted)', cursor: 'pointer', borderTop: '1px solid var(--border)' }}
              >
                <LogOut size={14} /> Sair
              </button>
            </div>
